...
**The application can be accessed here : ** <fill url>
...
Originally, since the data records the owners' name and address, its use is covered by specific legislation and must be treated with special care. For this reason, the cleaned dataset has been aggregated by municipality. Data is available starting from 2015 with several months latency for the latest year.
...
* `categorie`: the type of electricity generation technology according to DGE-DIREN nomenclature
...
* `consommation`: the amount of electrical energy, in kWh as fed to the app, consumed by the sector
...
4. A tab dedicated to theoretical building consumption data. Two visualisations are generated: an alluvial plot for consumption per affectation or usage, and a table for each alluvial data. The data can be downloaded in CSV or Excel format.

5. A report tab which offers the user to download a html `.Rmd` report of the different visualisations available in the app for the current municipalities selection. The report is very basic for now, but can easily be developed in the future.

6. A information tab which provides some context and a contact point for feedbacks and questions, as well as basic documentation for each tab.
...
* [ ] Add tabInd for main indicateurs (elec shares, regener main outputs)

  First things first: make tables of what data to display (see with CPD)

  When done : Use boxes layout to allow to fold/unfold communes ?


* [ ] Adding buildings potential for renovation

* [ ] Update tabMap's statboxes for buildings consumption

* [ ] Adding basic statistics about RegEner (m2 SRE, etc.)

* [ ] Add graph about buildings ages and relative consumption (IBC)

* [x] Add MWh as default 


## b) Medium priority

* [ ] Adding a 'per district' view

* [ ] Adding some more details about sectorial split for electricity consumption

This one is quite time intensive to make, to what use ?



## c) Small easy fixes

* [x] Buildings' heat: put some text in bold about the perimeter (surface heating + hot water only)

# 8) Known issues

* `bs4Dash::notificationItem()`'s `href` argument does not work in the current (28.11.2022) CRAN version, but is fixed in the dev version
